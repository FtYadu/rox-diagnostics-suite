...
const main = () => {
  ...
  const seed = {
    ...
    processes: set.processes.processes.map((process) => ({
      ...
      ...(process.securityAlg === undefined ? {} : { securityAlg: process.securityAlg }),
      requiresVin: process.requiresVin ?? false,
      ...(process.description ? { description: process.description } : {}),
      ...(process.market ? { market: process.market } : {}),
      ...(process.sourceFile ? { sourceFile: process.sourceFile } : {}),
